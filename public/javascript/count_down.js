...
window.addEventListener('load', function(){
  // 最新投稿の定義
  const lastReview = document.getElementById("limit_date")
  // 設定したタイムリミットを取得し、エポックミリ秒換算
  const limitId = lastReview.getAttribute("data-limit_id")
  const oneDay = 24 * 60 * 60 * 1000;
  const days = oneDay * limitId;
  // 最新の投稿日時を定義
  let createDt = lastReview.getAttribute("data-created_at");
